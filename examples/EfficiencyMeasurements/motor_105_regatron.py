...
def run():
    ...
    min_output_voltage = 200
    output_voltage_step = 100
    
    max_output_power = 3000
    min_output_power = 100
    step_output_power = 500
    ...
    loop_v = np.zeros((1,2))
    direction = 1
    for ii, power in enumerate(output_powers):
        direction = (direction+1)%2
        for jj, voltage in enumerate(input_voltages):
            if direction == 0:      
                loop_v=np.vstack((loop_v,[power, input_voltages[jj]]))
            elif direction == 1:
                loop_v=np.vstack((loop_v,[power, input_voltages_rev[jj]]))
    ...
    for row in loop_v:
        ...
        if ask_user() == True:
            ...
            try:
                ...
                print('Source Voltage set: %f' % -float(power/(voltage+eps)))
                source.powerOn()
                # Or use source.ramp_to_current(current, delay=0.1)
                sleep(2)
                sink.setCurrent(neg=-float(power/(voltage+eps)), pos=0)
                print('Sink Current set: %f' % -float(power/(voltage+eps)))
                print('==SINK READOUT==============================')
                print('VQ1_sink[V] = %f' % sink.getVoltagePreset())
                print('IQ1_sink[A] = %f' % sink.getCurrentPreset())
                print('IQ4_sink[A]  = %f' % sink.getCurrentLimitQ4())
